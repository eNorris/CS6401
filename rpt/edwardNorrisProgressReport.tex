...
\section{Background}
Background text. What other methods have been used to solve this problem?
...
Before the fitness of an individual can be calculated, teams must be formed. Each team consists of a single individual from each poplulation. Currently, teams are formed at random, in the future an archive method will be implemented. The fitness of a team is then equal to the figure of merit (FOM) of that solution, given in Eq. \ref{eq:fom}, where $T$ is the wall time to evaluate the solution and $\bar{R}$ is the average of the reported estimated relative uncertainty for all regions of interest by MCNP.
\begin{equation} \label{eq:fom}
FOM_{s,i} = \frac{1}{T_i\bar{R_i}^2}
\end{equation}
